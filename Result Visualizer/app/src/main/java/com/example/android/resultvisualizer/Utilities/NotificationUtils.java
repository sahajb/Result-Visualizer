...
public class NotificationUtils {
    ...
    public static void notification(Context context, String s) {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context, "notification-channel")
                ...
                .setLargeIcon(BitmapFactory.decodeResource(context.getResources(), R.mipmap.ic_launcher_result_visualizer_round))
                .setContentTitle("Result Available").setContentText("The result for " + s + " is now available").setStyle
                        (new NotificationCompat.BigTextStyle().bigText("The result for " + s + " is now available")).setDefaults(
                        Notification.DEFAULT_VIBRATE).setContentIntent(contentIntent(context)).addAction(dismiss(context)).
                        setAutoCancel(true);
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O)
