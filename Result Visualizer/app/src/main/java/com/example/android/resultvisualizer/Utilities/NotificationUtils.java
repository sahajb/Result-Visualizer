...
public class NotificationUtils {
    ...
    public static void notification(Context context, String s) {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context, "notification-channel").
                ...
                .setContentIntent(contentIntent(context)).addAction(dismiss(context)).addAction(view(context)).setAutoCancel(true);
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O)
    ...
    private static NotificationCompat.Action view(Context context) {
        Intent intent = new Intent(context, NotificationService.class);
        intent.setAction("view");
        PendingIntent pendingIntent = PendingIntent.getService(context, 2, intent, PendingIntent.FLAG_UPDATE_CURRENT);
        return new NotificationCompat.Action(0, "View", pendingIntent);
    }

    private static PendingIntent contentIntent(Context context) {
        return PendingIntent.getActivity(context, 2, new Intent(context, ResultActivity.class),
